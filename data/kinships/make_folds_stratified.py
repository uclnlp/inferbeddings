...
def main(argv):
    ...
    for fold_no, (train_valid_idx, test_idx) in enumerate(skf):
        ...
        with open('stratified_folds/{}/kinships_train.tsv'.format(str(fold_no)), 'w') as f:
            f.writelines(['{}\n'.format(line) for line in train_lines])

        with open('stratified_folds/{}/kinships_valid.tsv'.format(str(fold_no)), 'w') as f:
            f.writelines(['{}\n'.format(line) for line in valid_lines])

        with open('stratified_folds/{}/kinships_test.tsv'.format(str(fold_no)), 'w') as f:
