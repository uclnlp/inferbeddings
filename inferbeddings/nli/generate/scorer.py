...
class Generator:
    def __init__(self, embedding_layer, token_to_index, lm_path='models/lm/', batch_size=32):
        ...
        cells = [cell_fn(self.rnn_size) for _ in range(self.num_layers)]
        self.lm_cell = rnn.MultiRNNCell(cells)

        self.lm_input_data_ph = tf.placeholder(tf.int32, [None, self.seq_length], name='input_data')
        self.lm_targets_ph = tf.placeholder(tf.int32, [None, self.seq_length], name='targets')
        ...
        with tf.variable_scope('rnnlm'):
            ...
            lm_inputs = tf.split(lm_emb_projection, self.seq_length, 1)
        ...
        lm_output = tf.reshape(tf.concat(lm_outputs, 1), [-1, self.rnn_size])
        ...
        self.lm_loss = S.sequence_loss_by_example(logits=[lm_logits], targets=[tf.reshape(self.lm_targets_ph, [-1])],
                                                  weights=[tf.ones([self.lm_batch_size * self.seq_length])])
        self.lm_cost = tf.reduce_sum(self.lm_loss) / self.lm_batch_size / self.seq_length
