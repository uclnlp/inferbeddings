...
def to_cmd(fold_no, c):
    command = './bin/triples-cli.py -a' \
              ' --train data/umls/stratified_folds/{}/umls_train.tsv.gz' \
              ' --valid data/umls/stratified_folds/{}/umls_valid.tsv.gz' \
              ' --test data/umls/stratified_folds/{}/umls_test.tsv.gz' \
              ' --nb-epochs {}' \
              ' --lr {}' \
              ' --nb-batches {}' \
              ' --model {}' \
...
def to_logfile(fold_no, cfg, path):
    outfile = '{}/umls_v1.fold={}.{}.log'.format(path, fold_no, to_str(cfg))
    return outfile
