...
for MODEL in $MODELS
do
    BEST_AUCPR_FILE=`ls $@ | xargs grep "\[valid\]" | grep $MODEL | grep "AUC-PR:" | grep -v 'clauses' | awk '{ print $3 " " $0}' | sort -nr | head -n 1 | sed -e ''s/":INFO:"/" "/g'' | awk '{ print $2 }'`
...
for MODEL in $MODELS
do
    BEST_AUCPR_FILE=`ls $@ | xargs grep "\[valid\]" | grep $MODEL | grep "AUC-PR:" | grep 'clauses' | awk '{ print $3 " " $0}' | sort -nr | head -n 1 | sed -e ''s/":INFO:"/" "/g'' | awk '{ print $2 }'`
    if [ -n "$BEST_AUCPR_FILE" ]
    then
        BEST_AUCPR_VALID=`cat $BEST_AUCPR_FILE | grep '\[valid\]' | grep 'AUC-PR' | awk '{ print $3 }'`
