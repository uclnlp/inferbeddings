...
def to_cmd(c, idx, _path=None):
    ...
    command = '{}/bin/nli-smart-cli.py -f -n -m ff-dam --batch-size 32 --dropout-keep-prob 0.8 ' \
              '--representation-size 200 --optimizer adagrad --learning-rate 0.05 -c 100 -i uniform ' \
              '--nb-epochs 100 --has-bos --has-unk -p ' \
              '-S --restore /home/ucacmin/workspace/inferbeddings/models/snli/dam_1/dam_1 --{} {} -P {} -E data/snli/generated/snli_1.0_contradictions_*.gz ' \
              ''.format(_path, c['rule_id'], c['weight'], c['adversarial_pooling'])
    return command


def to_logfile(c, path):
    outfile = "%s/legion_nli_asr_v2.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = '/home/ucacmin/Scratch/inferbeddings/logs/nli/asr/legion_nli_asr_v2/'
