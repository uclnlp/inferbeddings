...
def to_cmd(c, idx):
    command = 'PYTHONPATH=. python3 ./bin/nli-dsearch-reg-cli.py ' \
              ...
              '-S --restore models/snli/esim_1/esim_1 --{} {} -P {} ' \
              '-E data/snli/generated/snli_1.0_contradictions_*.gz ' \
              '--hard-save models/snli/esim_1/acl/batch_dsearch_reg_v2e/esim_1_{}'\
        .format(c['rule_id'], c['weight'], c['adversarial_pooling'], idx)
    return command


def to_logfile(c, path):
    outfile = "%s/batch_dsearch_reg_v2e.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = './logs/nli/acl/batch_dsearch_reg_v2e/'
    _path = './models/snli/esim_1/acl/batch_dsearch_reg_v2e/'
