...
def to_cmd(c, idx):
    command = 'PYTHONPATH=. python3 ./bin/nli-dsearch-reg-cli.py ' \
              ...
              '--hard-save models/snli/dam_1/acl/batch_dsearch_reg_v3_01/dam_1_01_{}'\
        .format(c['rule_id'], c['weight'], c['adversarial_pooling'], idx)
    return command


def to_logfile(c, path):
    outfile = "%s/batch_dsearch_reg_v3_01.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = './logs/nli/acl/batch_dsearch_reg_v3_01/'
    _path = './models/snli/dam_1/acl/batch_dsearch_reg_v3_01/'
