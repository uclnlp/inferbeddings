...
def to_cmd(c, idx):
    command = 'PYTHONPATH=. python3 ./bin/nli-dsearch-reg-cli.py ' \
              ...
              '-S --restore models/snli/dam_1_01/dam_1_01 --04 {} --05 {} --06 {} -P {} ' \
              '-E data/snli/generated/snli_1.0_contradictions_*.gz ' \
              '--hard-save models/snli/dam_1/acl/batch_dsearch_reg_v8_01/dam_1_01_{}'\
        .format(c['weight'], c['weight'], c['weight'], c['adversarial_pooling'], idx)
    return command


def to_logfile(c, path):
    outfile = "%s/batch_dsearch_reg_v8_01.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = './logs/nli/acl/batch_dsearch_reg_v8_01/'
    _path = './models/snli/dam_1/acl/batch_dsearch_reg_v8_01/'
