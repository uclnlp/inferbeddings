...
def to_cmd(c, idx):
    command = 'PYTHONPATH=. python3 ./bin/nli-dsearch-reg-cli.py ' \
              ...
              '--07 {} --08 {} --09 {} --10 {} --12 {} ' \
              '-P {} ' \
              '--hard-save models/snli/cbilstm_1/acl/batch_dsearch_reg_v12c/cbilstm_1_{}'\
...
def to_logfile(c, path):
    outfile = "%s/batch_dsearch_reg_v12c.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = './logs/nli/acl/batch_dsearch_reg_v12c/'
    _path = './models/snli/cbilstm_1/acl/batch_dsearch_reg_v12c/'
