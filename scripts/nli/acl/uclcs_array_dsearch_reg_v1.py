...
def to_cmd(c, idx):
    command = 'PYTHONPATH=. python3 ./bin/nli-dsearch-reg-cli.py -f -n -m ff-dam ' \
              ...
              '--hard-save models/snli/dam_1/acl/uclcs_dsearch_reg_v1/dam_1_{}'\
        .format(c['rule_id'], c['weight'], c['adversarial_pooling'], idx)
    return command


def to_logfile(c, path):
    outfile = "%s/uclcs_dsearch_reg_v1.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = '/home/pminervi/workspace/inferbeddings/logs/nli/acl/uclcs_dsearch_reg_v1/'
...
mkdir -p models/snli/dam_1/acl/uclcs_dsearch_reg_v1/
