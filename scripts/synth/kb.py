...
# NEW: modifications for inferbeddings, related to formulae
#      some bugs; make deterministic with seed
#      (not further tested or completed for arity different from 2)
...
class KB:
    ...
    def get_facts(self, key, dim, typ=None):
        ...
        if typ is None:
            return sorted(result)#sorted: to keep deterministic
        else:
            return sorted([f for f in result if f[2] == typ])
    ...
    def get_formulae_for_ntp_strings(self):
        result = []
        for body, head in self.get_formulae_for_ntp():
            if len(body) == 1:
                result.append("%s(%s, %s) :- %s(%s, %s)" % (head[0], head[1], head[2],
                                                            body[0][0], body[0][1], body[0][2]))
            elif len(body) == 2:
                arg1, arg2 = body
                result.append("%s(%s, %s) :- %s(%s, %s), %s(%s, %s)" % (head[0], head[1], head[2],
    ...
    def apply_formulae(self, test_prob=0.0, sampled_unobserved_per_true=1):
        ...
        while not done:
            ...
            if "inv" in self.__formulae:
                for (body, head) in self.get_formulae("inv")[2]:
                    facts = self.get_facts(body, 0)
                    done = done and self.add_tmp_inferred_fact(facts, head, self.inv_args)

            if "impl" in self.__formulae:
                for arity in range(1, 4):
                    if arity in self.get_formulae("impl"):
                        for (body, head) in self.get_formulae("impl")[arity]:
                            facts = self.get_facts(body, 0)
                            done = done and self.add_tmp_inferred_fact(facts, head,
                                                          self.id_args)

            if "impl_conj" in self.__formulae:
                for arity in range(1, 4):
            ...
            if "trans" in self.__formulae:
                for (body1, body2, head) in self.get_formulae("trans")[2]:
                    facts1 = self.get_facts(body1, 0)
                    facts2 = self.get_facts(body2, 0)
                    ...
                    done = done and self.add_tmp_inferred_fact(facts, head, self.id_args)
...
def test_formulae():
    kb = KB()
    kb.add_train("profAt", ("Sebastian", "UCL"))
    kb.add_train("researchGroup", ("Sebastian", "UCLMR"))
    kb.add_train("researchGroup", ("Tim", "UCLMR"))
    kb.add_train("lecturerAt", ("David", "UCL"))
    kb.add_train("hasResearcher", ("UCLMR", "Tim"))
    kb.add_train("colleagueOf", ("Isabelle", "Tim"))

    kb.add_formulae("inv", {2: [("hasResearcher", "researcherAt"), ("colleagueOf", "colleagueOf")]})
    kb.add_formulae("impl", {2: [("profAt", "lecturerAt")]})
    kb.add_formulae("trans", {2: [("profAt", "hasResearcher", "supervisorOf")]})

    print('\nfacts:')
    for f in kb.get_all_facts():
        print(f)
    print('\nformulae:')
    for f in kb.get_formulae_for_ntp_strings():
        print(f)
    print('\nfacts after apply_formulae and sampling unobserved')
    kb.apply_formulae()
    for f in kb.get_all_facts():
        print(f)
