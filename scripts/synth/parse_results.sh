...
TAGS="symm impl impl_inv trans_single trans_diff"
#TAGS="trans_single trans_diff"
CONFS="0.0 0.2 0.4"
VERSION="0"
#CONFS="0.3 0.5 0.7"
    ...
    for CONF in $CONFS; do
        ...
        for MODEL in $MODELS; do
            ...
            for SS in $SUBSAMPLES; do
                BEST_AUCPR_VALID=`ls $(pwd) | xargs grep "\[valid\]" | grep "AUC-PR" | grep "$TAG" | \
                    grep "$1" | grep "$2" | grep "$3" | \
                    grep "adv_weight=0_" | grep "model=$MODEL" | \
                    grep "subsample_size=$SS" | grep "embedding_size=" | grep "c${CONF}_v${VERSION}" | \
