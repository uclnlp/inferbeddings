...
results = '/Users/tdmeeste/workspace/inferbeddings/logs/synth/synth_paper_aggregated.txt'
...
adv_weights_lst = ['0', '1']
adv_epochs_lst = ['0', '1', '10']
disc_epochs_lst = ['1', '10']
...
with open(results) as rID:
    for line in rID:
        ...
        if not None in (clause, model, adv_init_ground, conf, adv_weight, adv_epochs, disc_epochs, version):
            ID2AUC[(clause, model, adv_init_ground, conf, adv_weight, adv_epochs, disc_epochs, version)] = float(auc)


ID2AUC_versions = {}
for (clause, model, adv_init_ground, conf, adv_weight, adv_epochs, disc_epochs, version), auc in ID2AUC.items():
    if not (clause, model, adv_init_ground, conf, adv_weight, adv_epochs, disc_epochs) in ID2AUC_versions:
        ID2AUC_versions[(clause, model, adv_init_ground, conf, adv_weight, adv_epochs, disc_epochs)] = []
    ID2AUC_versions[(clause, model, adv_init_ground, conf, adv_weight, adv_epochs, disc_epochs)].append(auc)
...
\begin{table}[t!]
\centering
\caption{ PR-AUC results for STD, $\text{\ASR}^{\tau_{a} = 0}$, and ASR on synthetic datasets with various types of clauses (with $r\not=s\not=t$).}
\label{synth}
\vspace{1em}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{llccc}
    \toprule
Clauses & Model & STD & $\text{\ASR}^{\tau_{a} = 0}$ & ASR \\
\midrule
"""
footer = r"""
\bottomrule
...
def results_line(clause, model):
    res = string(model) + " & "
    conf = "0.0"
    res_STD = ID2MEAN[(clause, model, True, conf, '0', '0', '10')]
    res_SMPL = ID2MEAN[(clause, model, True, conf, '1', '0', '10')]
    #res_ASR_R = ID2MEAN[(clause, model, False, conf, '1', '1')]
    res_ASR_G = ID2MEAN[(clause, model, True, conf, '1', '10', '10')]
    resu = [res_STD, res_SMPL, res_ASR_G]
