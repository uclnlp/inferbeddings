...
def to_cmd(c, _path=None):
    ...
    command = 'python3 {}/bin/kbp-cli.py' \
              ' --train {}/data/schematic-memory/WN18RR/train.txt' \
              ' --valid {}/data/schematic-memory/WN18RR/valid.txt' \
              ' --test {}/data/schematic-memory/WN18RR/test.txt' \
              ' --nb-epochs {}' \
              ' --lr 0.1' \
              ' --nb-batches 10' \
              ' --model {}' \
...
def to_logfile(c, path):
    outfile = "%s/ucl_wn18rr_v1.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    path = '/home/pminervi/workspace/inferbeddings/logs/schematic-memory/filtered/ucl_wn18rr_v1/'
