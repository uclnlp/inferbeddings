...
def to_logfile(c, path):
    outfile = "%s/ucl_music_adv_xshot_v1.%s.log" % (path, summary(c))
    return outfile
...
def main(argv):
    ...
    hyperparameters_space_transe = dict(
        ...
        loss=['hinge'],
        subsample_size=[.1, .2, .3, .4, .5, .6, .7, 1],
        adv_lr=[.1],
        adv_epochs=[0, 10],
        disc_epochs=[10],
    ...
    path = '/home/pminervi/workspace/inferbeddings/logs/ucl_music_adv_xshot_v1/'
    ...
    for job_id, cfg in enumerate(configurations):
        ...
        if not completed:
            ...
            if args.debug:
                ...
                file_name = 'ucl_music_adv_xshot_v1_{}.job'.format(job_id)
                alias = ''
