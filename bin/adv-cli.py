...
from inferbeddings.adversarial.base import GenerativeAdversarial
...
def train(session, train_sequences, traing_targets, nb_entities, nb_predicates, nb_batches, seed, similarity_name,
          entity_embedding_size, predicate_embedding_size, hidden_size,
          model_name, loss_name, pairwise_loss_name, margin, learning_rate, initial_accumulator_value, nb_epochs,
          parser,
          ...
          entity_embeddings_provider=default_entity_embeddings,
          predicate_embeddings_provider=default_predicate_embeddings,
          corrupt_for_neg_data=True,
          args=None):
    ...
    # object_corruptor = corrupt.InverseCorruptor
    ...
    X_target = np.array(traing_targets, dtype=np.float32)
    ...
    predicate_embeddings = provided_predicate_embeddings.embeddings
    ...
    nb_versions = 3 if corrupt_for_neg_data else 1
    ...
    if predicate_l2 is not None:
        fact_loss += tf.nn.l2_loss(provided_predicate_embeddings.embedding_matrix)

    loss_function += fact_loss

    if tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES):
        loss_function += tf.add_n(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))

    # fact_loss += slim.losses.get_regularization_losses()

    # Optimization algorithm being used.
    optimizer = tf.train.AdagradOptimizer(learning_rate=learning_rate,
                                          initial_accumulator_value=initial_accumulator_value)
    trainable_var_list = provided_entity_embeddings.trainable_variables + \
                         provided_predicate_embeddings.trainable_variables + \
                         model.get_params()
    ...
    projection_steps = provided_entity_embeddings.projection_steps + provided_predicate_embeddings.projection_steps
    if predicate_norm is not None:
        projection_steps += [
            constraints.renorm_update(provided_predicate_embeddings.embedding_matrix, norm=predicate_norm)]
    ...
    for epoch in range(1, nb_epochs + 1):
        ...
        for disc_epoch in range(1, discriminator_epochs + 1):
            ...
            if corrupt_for_neg_data:
                ...
                Xr_sc, Xe_sc, Xr_oc, Xe_oc = None, None, None, None
                X_target_shuffled = X_target[order]
            ...
            for batch_start, batch_end in batches:
                ...
                if corrupt_for_neg_data:
                    ...
                    X_target_batch = X_target_shuffled[batch_start:batch_end]
...
def main(argv):
    ...
    argparser.add_argument('--no-corruption', action='store_true',
                           help='Should we avoid the use of corruption to generate extra negative data', default=False)
    ...
    if args.debug_scores is not None:
        ...
        for path in args.debug_scores:
            ...
            debug_facts += [fact(s, p, o) for s, p, o in debug_triples]
    ...
    parser = KnowledgeBaseParser(
        train_facts + valid_facts + test_facts + train_facts_neg + valid_facts_neg + test_facts_neg + debug_facts)
    ...
    train_sequences = parser.facts_to_sequences(train_facts)  # + parser.facts_to_sequences(train_facts_neg)
    train_targets = [1.0] * len(train_facts)  # + [0.0] * len(train_facts_neg)
    ...
    with tf.Session(config=sess_config) as session:
        scoring_function, objects = train(session, train_sequences, train_targets,
                                          nb_entities, nb_predicates, nb_batches, seed,
                                          similarity_name,
                                          entity_embedding_size, predicate_embedding_size, hidden_size,
                                          ...
                                          corrupt_for_neg_data=not args.no_corruption,
                                          args=args)
